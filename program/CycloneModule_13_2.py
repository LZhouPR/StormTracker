...
class cyclonefield:
    ...
    def findMinima(self, field, mask, kSize, nanthreshold=0.5):
        ...
        '''
        self.fieldMinima = detectMinima(field,mask,kSize,nanthreshold).astype(np.int64)
...
def ringDistance(ydist, xdist, rad):
    ...
    kradius = np.int64( np.ceil( rad / min(ydist,xdist) ) )
...
def trackCyclones(cfa,cfb,ctr,maxdist,red,tmstep):
    ...
    for c,cyc in enumerate(cf1.cyclones):
        ...
        if len(cyct.data) == 1: # If cf1 represents the genesis event, first guess is no movement
            ...
            latq = addLat(cyc.lat,dist2lat(float(red*cyct.data.loc[cyct.data.time == cyc.time,"v"].iloc[0])*tmstep))
            lonq = addLon(cyc.lon,dist2lon(float(red*cyct.data.loc[cyct.data.time == cyc.time,"u"].iloc[0])*tmstep,cyc.lat,cyc.lat))
    ...
    for id2 in range(len(mc2)):
        ...
        if len(mc2[id2]) > 1:
            ...
            ptid1 = int(ct[cf1.cyclones[id1].tid].data.loc[ct[cf1.cyclones[id1].tid].data.time == cf1.time,"ptid"].iloc[0])
            ...
            if sum(ptid_test) > 0: # if the ptid is the tid of one of the other candidates, merge into the parent instead
                id1 = int(ct[ptid1].data.loc[ct[ptid1].data.time == time1,"id"].iloc[0])
    ...
    for id2 in range(len(sc2)):
        ...
        if len(mc2[id2]) == 0 and len(sc2[id2]) > 0: # if there's one or more centers that could have tracked there -> SPLIT
            ...
            id2_1 = int(ct[cf1.cyclones[id1].tid].data.loc[ct[cf1.cyclones[id1].tid].data.time == time2,"id"].iloc[0])
    ...
    for cy2 in cf2.cyclones:
        ...
        if int(ct[cy2.tid].data.loc[ct[cy2.tid].data.time == time2,"centers"].iloc[0]) != 1:
            ...
            if len(ct[cy2.tid].data.loc[(ct[cy2.tid].data.time == time1) & (ct[cy2.tid].data.type != 0)]) == 0:
                ...
                cy2_id1 = int(ct[cy2.tid].data.loc[ct[cy2.tid].data.time == time1,"id"].iloc[0])
                ...
                for ti in  ma_tids: # for each track id
                    if len(ct[ti].data.loc[ct[ti].data.time == time1,"id"]) == 0:
                        ...
                        ma_id1 = int(ct[ti].data.loc[ct[ti].data.time == time1,"id"].iloc[0] )# Find the center id for cf1
    ...
    for cy1 in cf1.cyclones:
        ...
        if int(ct[cy1.tid].data.loc[ct[cy1.tid].data.time == time1,"centers"].iloc[0]) != 1:
            # What was the primary track in time 1?
            ptid1 = int(ct[cy1.tid].data.loc[ct[cy1.tid].data.time == time1,"ptid"].iloc[0])
            ...
            if len(ct[cy1.tid].data.loc[(ct[cy1.tid].data.time == time2) & (ct[cy1.tid].data.type != 0)]) == 0:
                ...
                if cy1.tid == ptid1:
                    ...
                    if sum(survtest) > 0:
                        ...
                        mcc_ids_surv_p = [float(ct[cf1.cyclones[i].tid].data.loc[ct[cf1.cyclones[i].tid].data.time == time2,"p_cent"].iloc[0]) \
                        ...
                        reid2 = int(ct[cf1.cyclones[reid1].tid].data.loc[ct[cf1.cyclones[reid1].tid].data.time == time2,"id"].iloc[0])
            ...
            # PART 4.3. AREA SPLIT #
                ...
                c1_id2 = int(ct[cy1.tid].data.loc[ct[cy1.tid].data.time == time2,"id"].iloc[0])
                ...
                for ti in  sa_tids: # for each track id
                    if len(ct[ti].data.loc[(ct[ti].data.time == time2) & (ct[ti].data.type != 0),"id"]) == 0:
                        ...
                        sa_id2 = int(ct[ti].data.loc[ct[ti].data.time == time2,"id"].iloc[0] )# Find the center id for cf2
    ...
    for cy2 in cf2.cyclones:
        ...
        if int(ct[cy2.tid].data.loc[ct[cy2.tid].data.time == time2,"centers"].iloc[0]) > 1:
            ...
            pr_mtids = [ti for ti in mtids if len(ct[ti].data.loc[ct[ti].data.time == time1,"type"]) > 0 \
                    and int(ct[ti].data.loc[ct[ti].data.time == time1,"type"].iloc[0]) > 0]
            ...
            # If more than one track existed in cf1 time,
                ...
                mp2s_pr = [float(ct[ti].data.loc[ct[ti].data.time == time2,"depth"].iloc[0]) for ti in pr_mtids]
                ...
                try:
                    ptid2 = int(ct[tid2].data.loc[ct[tid2].data.time == time2,"ptid"].iloc[0]) # identify its ptid as ptid for system
                except:
            ...
            for mtid in mtids:
                ...
                if ct[mtid].tid != ptid2 and int(ct[mtid].data.loc[ct[mtid].data.time == time2,"Emg"].iloc[0]) == 2:
                    ct[mtid].addEvent(cf2.cyclones[int(ct[mtid].data.loc[ct[mtid].data.time == time2,"id"].iloc[0])],time2,"ly",2,otid=ptid2)

        # For center-only lysis events, set the final ptid to match the last observed ptid
        if int(ct[cy2.tid].data.loc[ct[cy2.tid].data.time == time2,"Ely"].iloc[0]) == 1:
    ...
    for t in ct: # Loop through each original tracks
        ...
        # But if the ptid is sometimes equal to the tid, the track needs to be split up
            ...
            for r in t.data.time:
                ...
                if ( (t.tid == int(rdata["ptid"].iloc[0])) or (t.ftid == int(rdata["ptid"].iloc[0])) ) and ( int(rdata["type"].iloc[0]) != 0 ):
    ...
    if rg == 1:
        ...
        for t in cs:
            ...
            if rgs > 0:
                ...
                rg_otids.append(int(t.events.loc[t.events.event == "rg","otid"].iloc[0]))
