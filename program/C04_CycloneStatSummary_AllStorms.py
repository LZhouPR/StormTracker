...
# while mt[0] <= endtime[0] and mt[1] <= endtime[1]: # Simon changed
    ...
    trs = [c for c in cs if ((c.lifespan() > minlifespan) and (c.trackLength() >= mintracklength) and (np.max(c.data.lat) >= minlat) and (c.maxDistFromGenPnt() >= mindisp))]
    trs0 = [c for c in cs0 if ((c.lifespan() > minlifespan) and (c.trackLength() >= mintracklength) and (np.max(c.data.lat) >= minlat) and (c.maxDistFromGenPnt() >= mindisp))]
    trs2 = [c for c in cs2 if ((c.lifespan() > minlifespan) and (c.trackLength() >= mintracklength) and (np.max(c.data.lat) >= minlat) and (c.maxDistFromGenPnt() >= mindisp))]
